...
# Place the directive that your compiler uses for line control here.
# Some pre-ANSI compiler use "#line", but most use simply "#".
#
LINE = -DLINE=\"\#line\"

# If your compiler's provides an stdarg.h file that is incompatible
# with unproto's one, define STDARG here:
#
STDARG = -DSTDARG
...
OPTS=	$(PIPE) $(SKIP) $(BELL) $(MAP) $(ALIAS) $(LINE) $(STDARG)

CFLAGS	= $(OPTS)
#CFLAGS	= $(OPTS) -p -Dstatic=
#CFLAGS	= $(OPTS) -g -DDEBUG
...
tar:	clean
	(cd ..; tar cvf unproto-1.7.tar unproto-1.7)
	mv ../unproto-1.7.tar .
	compress unproto-1.7.tar
...
install:
	strip unproto
	cp unproto /usr/local/bin
	cp acc.mwc /usr/local/bin/acc
	chmod 755 /usr/local/bin/acc
